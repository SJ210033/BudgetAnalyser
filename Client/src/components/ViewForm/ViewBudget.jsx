...
function ViewBudget() {
  ...
  const currentRows = budgetData.slice(indexOfFirstRow, indexOfLastRow);
  ...
  useEffect(() => {
    axios
      .get('http://localhost:5500/getbudgetsum/' + projectID)
      .then((res) => setBudgetSum(res.data[0].budgetAmount))
      .catch((err) => console.log(err));
  }, []);
  ...
  useEffect(() => {
    axios
      .get('http://localhost:5500/getlistsubcategorymaindata/' + projectID)
  ...
  useEffect(() => {
    axios
      .get('http://localhost:5500/getbudgetdetailsmaindata/' + projectID)
      .then((res) => setBudgetData(res.data))
      .catch((err) => console.log(err));
  }, [reducerValue]);

  const handleDelete = (id) => {
    axios.delete(`http://localhost:5500/deletebudgetdetailsdata/${id}`);
    forceUpdate();
  }

  return (
    <div className='d-flex flex-column justify-content-center align-items-center'>
      <center className='mt-4'><h3>BUDGET DETAILS</h3></center>

      <h4 className='mt-3'>TOTAL BUDGET : <span style={{ color: 'green' }}>₹ {budgetSum}</span></h4>
      ...
      {(budgetData.length > 0) ?
          ...
          <table>
            ...
            {currentRows.map((m) =>
              <tr style={{ borderRight: '1px solid grey', borderLeft: '1px solid grey' }}>
                ...
                <td style={{ borderBottom: '1px solid grey' }} className='px-5 py-2'>{subCategoryListData.find((p) => p.listSubCategoryID === m.listSubCategoryID)?.listSubCategoryName}</td>
                <td style={{ borderBottom: '1px solid grey' }} className='px-5 py-2'><b>₹ {m.budgetAmount}</b></td>
                <td style={{ borderBottom: '1px solid grey' }} className='px-5 py-2'>
                  <FontAwesomeIcon onClick={() => setId(m.budgetID)} className='mx-4' style={{ cursor: 'pointer', color: 'blue', fontSize: '1.3rem' }} icon={faPenToSquare} />
                  <FontAwesomeIcon onClick={() => handleDelete(m.budgetID)} className='me-4' style={{ cursor: 'pointer', color: 'red', fontSize: '1.3rem' }} icon={faTrashCan} />
                </td>
              </tr>
            )}
          </table>
        ...
        <center style={{ border: '.2px solid grey', height: '200px', width: '300px' }} className='mt-5 pt-5'>
          ...
          <Link to={`/dashboard/${projectID}/addbudget`}><button className='btn btn-primary mt-4'>ADD BUDGET</button></Link>
        </center>
      }
      ...
      <div className="mt-4 d-flex justify-content-center">
        <nav aria-label="Page navigation">
          <ul className="pagination">
            {Array.from({ length: Math.ceil(budgetData.length / rowsPerPage) }, (_, index) => (
